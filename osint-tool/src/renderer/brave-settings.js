document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('save-urlscan-key')?.addEventListener('click', () => saveProviderKey({
    inputId: 'urlscan-api-key', settingName: 'urlscanApiKey', label: 'urlscan.io'
  }));
  document.getElementById('clear-urlscan-key')?.addEventListener('click', () => clearProviderKey({
    inputId: 'urlscan-api-key', clearName: 'clearUrlscanApiKey', label: 'urlscan.io'
  }));
  document.getElementById('save-virustotal-key')?.addEventListener('click', () => saveProviderKey({
    inputId: 'virustotal-api-key', settingName: 'virusTotalApiKey', label: 'VirusTotal'
  }));
  document.getElementById('clear-virustotal-key')?.addEventListener('click', () => clearProviderKey({
    inputId: 'virustotal-api-key', clearName: 'clearVirusTotalApiKey', label: 'VirusTotal'
  }));
  ...
  if (toolsGrid && !document.getElementById('tools-passive-state')) {
    toolsGrid.insertAdjacentHTML('beforeend', `
      <article class="panel tool-panel">
        <div class="panel-heading"><div><h3>Passive Infrastructure Enrichment</h3><p>Wayback + IP RDAP + urlscan.io + VirusTotal.</p></div><span id="tools-passive-state" class="status-ready">2 built-in</span></div>
        <p class="panel-note">يعمل على domain وpublic IP المشتقين من بيانات الحالة. Wayback وIP RDAP جاهزان بدون مفتاح؛ urlscan وVirusTotal اختياريان بمفاتيح محفوظة بأمان.</p>
      </article>`);
  }

  const formHint = document.getElementById('form-hint');
  if (formHint) formHint.textContent = 'مفاتيح HIBP وBrave وurlscan وVirusTotal لا تُعاد إلى renderer بعد حفظها.';
}
...
async function refreshProviderStates() {
  try {
    ...
    if (passive) {
      const optional = Number(Boolean(settings.hasUrlscanApiKey)) + Number(Boolean(settings.hasVirusTotalApiKey));
      passive.textContent = `${2 + optional}/4 جاهز`;
      passive.className = optional === 2 ? 'status-ready' : 'status-muted';
    }
  } catch {
