...
class DatabaseManager {
  ...
  addLog(personId, moduleName, status, message) {
    const stmt = this.db.prepare(`
      ...
      VALUES (?, ?, ?, ?)
    `);
    stmt.run(personId, moduleName, status, redactString(message));
  }
  ...
  deletePerson(personId) {
    ...
    try {
      for (const table of ['relations', 'media', 'search_results', 'logs', 'social_accounts', 'breaches', 'domains']) {
        this.db.prepare(`DELETE FROM ${table} WHERE person_id = ?`).run(personId);
      }
      const result = this.db.prepare('DELETE FROM persons WHERE id = ?').run(personId);
      this.db.exec('COMMIT');
